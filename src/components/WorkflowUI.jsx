...
export function SubmissionSuccess({ title, reference, notification, children, onReset }) {
  return <div className="demo-complete panel workflow-success" role="status" aria-live="polite" tabIndex="-1">
    <Icon name="check" size={34}/><span className="eyebrow">Securely received</span><h2>{title}</h2><p>{children}</p>
    ...
    {onReset && <button className="button ghost" type="button" onClick={onReset}>Send another</button>}
  </div>
}
