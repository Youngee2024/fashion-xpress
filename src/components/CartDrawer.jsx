...
export function CartDrawer({ open, items, onClose, onRemove, onQuantityChange }) {
  ...
  useEffect(() => {
    if (open && checkoutStage !== 'bag') stageHeadingRef.current?.focus()
  }, [open, checkoutStage])
  ...
  return <div className="drawer-shell open">
    ...
    <aside ref={dialogRef} className="drawer" role="dialog" aria-modal="true" aria-labelledby="cart-title" aria-describedby="cart-description" tabIndex="-1">
      <div className="drawer-head"><div><span className="eyebrow">Your local edit</span><h2 id="cart-title">Bag ({itemCount})</h2><p id="cart-description" className="sr-only">A local prototype bag. Checkout makes no payment or order.</p></div><button ref={closeRef} className="icon-button" onClick={closeAndRestoreFocus} aria-label="Close shopping bag"><Icon name="close"/></button></div>
      {checkoutStage === 'bag' ? <>
        ...
        <div className="drawer-total"><div><span>Concept total</span><strong>{total.toFixed(1)} ETH</strong></div><button className="button full" onClick={() => setCheckoutStage('review')} disabled={items.length === 0}>Explore checkout prototype</button><small>No payment, wallet connection, or order will occur. Bag contents stay only in this browser.</small></div>
      </> : <div className="checkout-prototype" role="status" aria-live="polite"><span className="eyebrow">Portfolio prototype</span>{checkoutStage === 'review' ? <><h3 ref={stageHeadingRef} tabIndex="-1">Review the concept checkout</h3><p>{itemCount} {itemCount === 1 ? 'piece' : 'pieces'} · {total.toFixed(1)} ETH concept total</p><p>This is a presentation step only. No payment, wallet connection, or order will occur.</p><button className="button full" onClick={() => setCheckoutStage('complete')}>Complete checkout demo</button><button className="text-link" onClick={() => setCheckoutStage('bag')}>Back to bag</button></> : <><h3 ref={stageHeadingRef} tabIndex="-1">Checkout demo complete</h3><p>No payment was taken. No order was placed. No wallet was connected.</p><button className="button full" onClick={() => setCheckoutStage('bag')}>Return to bag</button></>}</div>}
    </aside>
  </div>
}
