...
export function Privacy() {
  return <InformationPage notice={false} eyebrow="Privacy notice" title="Privacy, in plain language." intro={IS_DEMO_MODE ? 'This portfolio is in Demo Mode. Contact, creator, newsletter, and Community journeys can be explored without real accounts or online publication.' : 'This deployment is in Live Mode. FashionXpress uses Supabase Auth for Community accounts and protected server workflows for contact, creator applications, and newsletters.'} sections={IS_DEMO_MODE ? [
    ...
    { title: 'Other local experiences', copy: ['Cart identifiers and quantities persist in this browser. Checkout step choices and Demo Collection records use session-only storage; they contain fictional identity presets, product IDs, quantities, concept licences, dates, and demo references, but no real personal information or payment credentials. The fictional Demo Community stays in page memory only.', 'Camera preview and capture stay on your device; a frame is saved only if you choose to download it. AR selection is conceptual: no garment tracking or fitting occurs. Checkout and minting create no payment, real order, ownership transfer, wallet connection, blockchain transaction, or token.'] },
    ...
    { title: 'What stays on your device', copy: ['Cart product choices and quantities are stored locally. Checkout step choices and Demo Collection records remain in session storage; they carry no personal or payment details and have no monetary or blockchain value. Camera frames remain on your device unless you choose to download one. The site does not currently operate analytics.'] },
...
export function Terms() {
  return <InformationPage eyebrow="Prototype terms" title="Terms for exploring the concept." intro="This site demonstrates a possible digital-fashion experience. It does not offer a live marketplace or create a commercial relationship." sections={[
    ...
    { title: 'No transactions', copy: ['Checkout and minting experiences are non-transactional. No payment, real order, licence, ownership record, token, or delivery is created. Demo Collection pieces have no monetary or blockchain value.'] },
...
export function RefundPolicy() {
  return <InformationPage eyebrow="Purchase status" title="Purchases are unavailable." intro="There is currently nothing to refund because FashionXpress does not accept payments, connect wallets, or deliver digital products." sections={[
    { title: 'Prototype checkout', copy: ['Cart, checkout, Demo Collection, and mint screens demonstrate interface states only. Completing checkout creates no payment, real order, ownership, or licence transfer. Demo Collection pieces have no monetary or blockchain value; minting creates no token or blockchain transaction. Live payment support is not enabled.'] },
