...
export function Privacy() {
  return <InformationPage notice={false} eyebrow="Privacy notice" title="Privacy, in plain language." intro={IS_DEMO_MODE ? 'This portfolio is in Demo Mode. Contact, creator, newsletter, and Community journeys can be explored without real accounts or online publication.' : 'This deployment is in Live Mode. FashionXpress uses Supabase Auth for Community accounts and protected server workflows for contact, creator applications, and newsletters.'} sections={IS_DEMO_MODE ? [
    ...
    { title: 'Other local experiences', copy: ['Cart identifiers and quantities persist in this browser. Digital Wardrobe checkout records use session-only storage and contain product IDs, illustrative licence choices, dates, and demo order references—not personal information or payment credentials. The optional Collectibles Lab uses a separate session-only namespace for its fictional Demo Wallet, progress, and collectible records.', 'Virtual Try-On camera streams, selected photos, manual overlays, and captured compositions stay in temporary device memory. Nothing is uploaded, persisted, analysed, body-tracked, or measured; downloaded images exist only when you explicitly save them. Collectibles Lab metadata is never uploaded or published.'] },
    ...
    { title: 'What stays on your device', copy: ['Cart product choices and quantities are stored locally. Digital Wardrobe checkout records remain in session storage and carry no personal or payment details; they do not represent an order or legal licence. The optional Collectibles Lab uses separate session-only storage for its fictional Demo Wallet and local records, which have no monetary or blockchain value. Virtual Try-On camera streams, photos, overlays, and captures stay in temporary device memory and are not uploaded, measured, or persisted; a file exists only if you download it. The site does not currently operate analytics.'] },
    { title: 'Features still unavailable', copy: ['Checkout is a guided commerce demo only. Payments, real orders, fulfilment, enforceable licences, wallet connectivity, blockchain activity, and garment tracking remain unavailable. Those prototype interfaces do not collect transaction data.'] },
  ]}/>
}
...
export function Terms() {
  return <InformationPage eyebrow="Prototype terms" title="Terms for exploring the concept." intro="This site demonstrates a possible digital-fashion experience. It does not offer a live marketplace or create a commercial relationship." sections={[
    { title: 'Demonstration and Community', copy: ['Prices, release tiers, planned licence allocations, product descriptions, and Demo Mode conversations are illustrative portfolio content. Planned allocations are not live inventory. Live Mode Community posts are supplied by members and should not be treated as commercial offers.', 'Members must follow the Community guidelines. Published content can be removed by its author or hidden through administrative review.'] },
    { title: 'Commerce prototype and optional Lab', copy: ['Naira checkout creates only a session-local Digital Wardrobe record. No payment, real order, delivered product, inventory reservation, or legal licence is created. Licence descriptions are illustrative and copyright remains with the creator.', 'Collectibles Lab and its fictional Demo Wallet are a separate optional educational simulation. No NFT, token, gas charge, signature, smart-contract execution, blockchain transaction, upload, publication, ownership, scarcity, or monetary value results from a demo collectible record.'] },
...
export function Licensing() {
  return <InformationPage eyebrow="Concept guide" title="Digital fashion licensing." intro="The current garments are demonstration pieces. No licence or ownership right is transferred through this prototype." sections={[
    ...
    { title: 'Creator rights', copy: ['Copyright remains with the creator. Creators would retain every right not explicitly granted in a final agreement. Planned allocations are illustrative, while collectible edition concepts in the optional Lab are not active contractual terms.'] },
    { title: 'Customer clarity', copy: ['Before any purchase, a production experience would show the exact files delivered, supported platforms, usage limits, and whether rights can be transferred.'] },
    ...
    { title: 'Optional Collectibles Lab terms', copy: ['Collectibles Lab choices and metadata are a separate local educational concept. Nothing is uploaded or made immutable, no asset is created on-chain, and no ownership, scarcity, resale, or licence right arises from a demo collectible record.'] },
  ]}/>
}
...
export function RefundPolicy() {
  return <InformationPage eyebrow="Purchase status" title="Purchases are unavailable." intro="There is currently nothing to refund because FashionXpress does not accept payments, connect wallets, or deliver digital products." sections={[
    ...
    { title: 'Separate Collectibles Lab', copy: ['The fictional Demo Wallet and demo collectibles are optional educational simulations inside Collectibles Lab. They make no gas charge, signature, smart-contract execution, upload, or blockchain transaction. Live payment and blockchain functionality are not enabled.'] },
...
export function AccessibilityStatement() {
  return <InformationPage eyebrow="Accessibility" title="Designed for more ways of navigating." intro="FashionXpress aims to make this portfolio prototype understandable and operable across keyboard, pointer, and assistive-technology use." sections={[
    { title: 'Current support', copy: ['The interface includes semantic headings, a skip link, visible keyboard focus, labelled controls, reduced-motion support, responsive layouts, and announced status feedback. Digital Wardrobe and Collectibles Lab navigation, item actions, and confirmed reset controls are keyboard operable. Virtual Try-On provides labelled sliders, buttons, pointer dragging, and arrow-key garment positioning as alternatives to touch gestures. Community forms report validation and submission states; one-time-code verification is keyboard operable.'] },
