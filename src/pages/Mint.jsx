...
function MintSteps({ current }) {
  return <nav className="mint-steps" aria-label="Demo collectible steps"><ol>{STEP_LABELS.map((label, index) => <li key={label} aria-current={index === current ? 'step' : undefined}><span>{String(index + 1).padStart(2, '0')}</span>{label}</li>)}</ol></nav>
}
...
function MintSummary({ summary, licence }) {
  return <div className="mint-summary panel"><dl><div><dt>Concept creation value</dt><dd>{formatDemoUnits(summary.mintUnits)}</dd></div><div><dt>Concept gas estimate</dt><dd>{formatDemoUnits(summary.gasUnits)}</dd></div><div><dt>Concept total</dt><dd>{formatDemoUnits(summary.totalUnits)}</dd></div><div><dt>Concept network</dt><dd>{summary.network.name} · fictional</dd></div><div><dt>Demo Wallet</dt><dd>{DEMO_WALLET.address}</dd></div><div><dt>Concept licence</dt><dd>{CONCEPT_LICENCES.find((item) => item.id === licence)?.label}</dd></div><div><dt>Collectible edition concept</dt><dd>{summary.metadata.edition}</dd></div></dl><p>All values are fictional demo units with no monetary value.</p></div>
}
...
export function Mint() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      try {
        ...
        navigate(`/collectibles/${id}/complete`, { replace: true })
      } catch (error) {
  ...
  if (!product || !draft) return <Navigate to="/collectibles" replace/>
  ...
  function resetWallet() {
    ...
    setMessage('Demo Wallet and collectible choices reset. Nothing external was affected.')
  }
  ...
  function cancelProgress() {
    ...
    setMessage('Demo collectible cancelled safely. Nothing was submitted or created.')
  }

  return <section className="page-shell mint-studio"><span className="eyebrow">Optional educational concept / Collectibles Lab</span><h1 ref={headingRef} tabIndex="-1">{STEP_LABELS[currentIndex]}.</h1><p className="mint-intro">A local collectible simulation for {product.name}, separate from normal Naira shopping. No wallet, blockchain, contract, or publishing service is connected.</p><p className="mint-entry">Entry: {draft.entrySource === 'checkout' ? 'Earlier checkout record' : draft.entrySource === 'vault' ? 'Collectibles Lab record' : draft.entrySource === 'product' ? 'Product detail exploration' : 'Collectibles Lab exploration'}.</p><MintSteps current={currentIndex}/><p className="sr-status" role="status" aria-live="polite">{message}</p>
    <div className="mint-studio-layout"><div className="mint-studio-main">
      ...
      {draft.step === 'review' && <><h2>Review the demo collectible</h2><MintSummary summary={summary} licence={draft.licence}/><fieldset className="mint-acknowledgement"><legend>Required acknowledgement</legend><label><input type="checkbox" checked={draft.acknowledged} onChange={(event) => setDraft((value) => ({ ...value, acknowledged: event.target.checked }))}/><span>I understand this is a local visual simulation and acknowledge every statement below.</span></label><ul>{DISCLOSURES.map((item) => <li key={item}>{item}</li>)}</ul></fieldset></>}
      {draft.step === 'progress' && <><h2>Demo collectible progress</h2><p>This sequence is a visual simulation. It does not contact a wallet, network, contract, storage service, or marketplace.</p><ol className="mint-progress" aria-label="Visual simulation progress">{PROGRESS_LABELS.map((label, index) => <li key={label} className={index <= draft.progressIndex ? 'active' : ''} aria-current={index === draft.progressIndex ? 'step' : undefined}><span>{index < draft.progressIndex ? 'Complete' : index === draft.progressIndex ? 'Demonstrating' : 'Waiting'}</span><strong>{label}</strong></li>)}</ol>{draft.progressIndex < 4 && <button className="button ghost" type="button" onClick={cancelProgress}>Cancel demo collectible</button>}</>}
      {draft.step !== 'progress' && <div className="checkout-controls">{currentIndex > 0 && <button className="button ghost" type="button" onClick={() => goTo(MINT_STEPS[currentIndex - 1])}>Back</button>}{draft.step === 'metadata' && <button className="button" type="button" onClick={() => goTo('wallet')}>Continue to Demo Wallet</button>}{draft.step === 'wallet' && <button className="button" type="button" disabled={!draft.walletActive} onClick={() => goTo('network')}>Continue to network</button>}{draft.step === 'network' && <button className="button" type="button" onClick={() => goTo('review')}>Review demo collectible</button>}{draft.step === 'review' && <button className="button" type="button" disabled={!draft.acknowledged || !draft.walletActive} onClick={() => goTo('progress')}>Create demo collectible</button>}<Link className="text-link" to={`/collections/${id}`}>Return to product</Link></div>}
...
export function MintComplete() {
  ...
  if (!product || !asset) return <Navigate to="/collectibles" replace/>
  const network = getConceptNetwork(asset.networkId)
  const licence = CONCEPT_LICENCES.find((item) => item.id === asset.licence)
  return <section className="page-shell mint-complete"><span className="eyebrow">Collectibles Lab / local visual simulation</span><h1 ref={headingRef} tabIndex="-1">Demo collectible complete.</h1><p role="status" aria-live="polite">Nothing was created on-chain. No wallet connected, contract executed, asset published, payment charged, token created, or ownership transferred.</p><div className="mint-complete-card panel"><SafeImage src={product.image} alt={product.name} width="320" height="420"/><dl><div><dt>Product</dt><dd>{product.name}</dd></div><div><dt>Creator</dt><dd>{product.creator}</dd></div><div><dt>Demo asset ID</dt><dd className="demo-identifier">{asset.assetId}</dd></div><div><dt>Fictional transaction reference</dt><dd className="demo-identifier">{asset.transactionReference}</dd></div><div><dt>Concept network</dt><dd>{network.name} · fictional</dd></div><div><dt>Collectible edition concept</dt><dd>{asset.edition}</dd></div><div><dt>Concept licence</dt><dd>{licence.label}</dd></div><div><dt>Completion time</dt><dd>{new Date(asset.completedAt).toLocaleString()}</dd></div><div><dt>Status</dt><dd>Not on-chain</dd></div></dl></div><div className="checkout-controls"><Link className="button" to="/collectibles">View in Collectibles Lab</Link><Link className="button ghost" to={`/collections/${id}`}>Return to product</Link><Link className="text-link" to="/collections">Explore Collections</Link><button className="text-link" type="button" onClick={() => { clearMintDraft(); navigate(`/collectibles/${id}?source=vault&step=metadata`) }}>Restart collectible demonstration</button></div></section>
}
