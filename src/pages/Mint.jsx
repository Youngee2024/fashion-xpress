...
export function Mint() {
  ...
  return <section className="page-shell mint-studio"><span className="eyebrow">Portfolio Demo / Mint Studio</span><h1 ref={headingRef} tabIndex="-1">{STEP_LABELS[currentIndex]}.</h1><p className="mint-intro">A local visual simulation for {product.name}. No wallet, blockchain, contract, or publishing service is connected.</p><p className="mint-entry">Entry: {draft.entrySource === 'checkout' ? 'Demo checkout concept item' : draft.entrySource === 'vault' ? 'Demo Vault concept item' : draft.entrySource === 'product' ? 'Product detail exploration' : 'Direct Mint Studio exploration'}.</p><MintSteps current={currentIndex}/><p className="sr-status" role="status" aria-live="polite">{message}</p>
