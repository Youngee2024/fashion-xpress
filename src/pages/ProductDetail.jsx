...
export function ProductDetail({ onAdd }) {
  ...
  return <section className="detail page-shell"><Link className="back-link" to="/collections">← Back to collection</Link><div className="detail-grid"><div className="detail-image"><SafeImage priority width="900" height="1350" src={product.image} alt={product.name}/><span>Concept edition of {product.stock}</span></div><div className="detail-copy"><span className="eyebrow">Genesis / {product.rarity}</span><h1>{product.name}</h1><p className="detail-price">{product.price.toFixed(1)} ETH <small>Concept price</small></p><p className="lead">{product.description}</p><div className="attribute"><div><span>Rarity concept score</span><strong>{product.score}/100</strong></div><div className="meter"><i style={{ width: `${product.score}%` }}/></div></div><dl><div><dt>Format concept</dt><dd>Wearable · AR-ready vision</dd></div><div><dt>Royalty concept</dt><dd>8% on resale</dd></div><div><dt>Network concept</dt><dd>Ethereum L2</dd></div></dl><div className="detail-actions"><button className="button full" onClick={() => onAdd(product)}>Add to local bag <Icon name="bag" size={17}/></button><Link className="button ghost full" to={`/mint/${product.id}`}>View mint experience</Link></div><Link className="text-link detail-ar-link" to="/ar-tryon">Explore camera concept →</Link><small>Portfolio listing · No checkout, license transfer, or blockchain transaction is available</small></div></div></section>
}
