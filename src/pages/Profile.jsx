...
import { PRESET_AVATARS, PROFILE_IDENTITIES, displayIdentity, hasValidationErrors, safeReturnTo, validateProfile } from '../data/communityRules'
...
export function ProfilePage() {
  ...
  return <section className="page-shell account-page"><span className="eyebrow">Community profile</span>{stage === 'loading' ? <p role="status">Loading profile…</p> : stage === 'unavailable' ? <div role="alert"><h1>Profile service unavailable.</h1><p>Live Supabase configuration is missing. No demo profile was substituted.</p></div> : stage === 'error' ? <div role="alert"><h1>Profile unavailable.</h1><p>Check your connection and retry.</p><button className="button" onClick={() => window.location.reload()}>Retry</button></div> : stage === 'missing' ? <div><h1>Profile not found.</h1><Link className="button" to="/community">Back to Community</Link></div> : <><div className="profile-hero panel"><span className={`profile-avatar avatar-${data.avatar_id}`} aria-hidden="true">{data.display_name?.[0] ?? 'F'}</span><div><h1>{data.display_name}</h1><p>@{data.handle} · {displayIdentity(data.identity)}</p><p>{data.bio || 'No bio yet.'}</p>{data.location && <small>{data.location}</small>}</div>{own && <div className="profile-actions"><Link className="button ghost" to="/profile/edit">Edit profile</Link><SignOutButton/></div>}</div><div className="profile-activity"><article><h2>Discussions</h2>{activity?.discussions?.length ? activity.discussions.map((item) => <Link className="activity-link" to={`/community/${item.id}`} key={item.id}>{item.title}<span>{new Date(item.created_at).toLocaleDateString()}</span></Link>) : <p>No published discussions yet.</p>}</article><article><h2>Replies</h2>{activity?.replies?.length ? activity.replies.map((item) => <Link className="activity-link" to={`/community/${item.discussion_id}`} key={item.id}>{item.body}<span>{new Date(item.created_at).toLocaleDateString()}</span></Link>) : <p>No published replies yet.</p>}</article></div></>}</section>
}
