...
const STEP_LABELS = ['Bag review', 'Customer profile', 'Digital-use licences', 'Payment demo', 'Final review']
...
export function Checkout({ cart, onQuantityChange, onRemove, onCompleted }) {
  ...
  return <section className="page-shell checkout-page"><span className="eyebrow">Portfolio Demo / Naira checkout</span><h1 ref={headingRef} tabIndex="-1">{STEP_LABELS[currentIndex]}.</h1><p className="checkout-intro">Explore a Naira-priced digital-fashion checkout and choose a digital-use licence. This remains a local interface demonstration, not a purchase.</p><nav className="checkout-steps" aria-label="Checkout steps"><ol>{STEP_LABELS.map((label, index) => <li key={label} aria-current={index === currentIndex ? 'step' : undefined}><span>{String(index + 1).padStart(2, '0')}</span>{label}</li>)}</ol></nav><p className="sr-status" role="status" aria-live="polite">{message}</p>
    {!cart.length ? <div className="panel checkout-empty"><h2>Your bag is empty.</h2><p>Choose a digital-fashion piece to begin the local Naira checkout demonstration. Existing Demo Vault records remain available separately.</p><Link className="button" to="/collections">Explore Collections</Link><Link className="text-link" to="/demo-collection">View Demo Vault</Link></div> : <div className="checkout-layout"><div className="checkout-main">
      ...
      {step === 'ownership' && <><h2>Choose a demo customer profile</h2><p>No shipping address, real name, email, or phone number is needed. This fictional profile is used only for the local interface journey.</p><fieldset className="checkout-options"><legend>Demo customer profile</legend>{DEMO_CUSTOMERS.map((customer) => <label key={customer.id} className={draft.owner === customer.id ? 'selected' : ''}><input type="radio" name="demo-customer" checked={draft.owner === customer.id} onChange={() => setDraft((value) => ({ ...value, owner: customer.id }))}/><span><strong>{customer.label}</strong><small>{customer.description}</small></span></label>)}</fieldset></>}
      {step === 'licence' && <><h2>Select digital-use licences</h2><p>Prices below use the approved Naira uplifts. These are illustrative choices, not legally active licences. See the <Link className="text-link" to="/licensing">Licensing page</Link> for the current prototype status.</p>{summary.lines.map(({ product }) => <fieldset className="checkout-options" key={product.id}><legend>{product.name}</legend>{DIGITAL_USE_LICENCES.map((licence) => <label key={licence.id} className={(draft.licences[product.id] ?? 'personal') === licence.id ? 'selected' : ''}><input type="radio" name={`licence-${product.id}`} checked={(draft.licences[product.id] ?? 'personal') === licence.id} onChange={() => setDraft((value) => ({ ...value, licences: { ...value.licences, [product.id]: licence.id } }))}/><span><strong>{licence.label} <em>{licence.adjustmentPercent ? `+${licence.adjustmentPercent}%` : 'Base price'}</em></strong><small>{licence.description}</small></span></label>)}</fieldset>)}</>}
      {step === 'payment' && <><h2>Demo Naira payment</h2><div className="demo-payment-card" aria-label="Fictional Naira payment demonstration"><span>NGN / DEMO</span><strong>{formatNaira(summary.totalKobo)}</strong><small>No payment details · Portfolio demonstration only</small></div><p>There are no card, bank, wallet, password, billing, or OTP fields. Nothing is sent to a payment provider.</p><Disclosure/></>}
      {step === 'review' && <><h2>Review the demonstration</h2><ItemList lines={summary.lines}/><p>Demo customer profile: {DEMO_CUSTOMERS.find((customer) => customer.id === draft.owner)?.label}</p><Disclosure/></>}
...
export function CheckoutComplete({ onRestart }) {
  ...
  return <section className="page-shell checkout-page checkout-complete"><span className="eyebrow">Portfolio Demo / order confirmation</span><h1 ref={headingRef} tabIndex="-1">Demo checkout complete.</h1><p role="status" aria-live="polite">No real payment was taken. No real order was created. No digital-use licence was transferred. No personal information was transmitted.</p><div className="panel"><p><strong>Demo order reference:</strong> {receipt.reference}</p><p><strong>Date:</strong> {new Date(receipt.completedAt).toLocaleString()}</p><ItemList lines={summary.lines}/><Totals summary={summary}/></div><Disclosure/><div className="checkout-controls"><Link className="button" to="/demo-collection">View Demo Vault</Link><Link className="button ghost" to="/collections">Return to Collections</Link><button className="text-link" onClick={() => { onRestart(receipt.items); navigate('/checkout?step=bag') }}>Restart demo checkout</button></div></section>
}
