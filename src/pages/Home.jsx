...
export function Home({ onAdd }) {
  return <>
    ...
    <section className="page-shell section featured-section"><SectionHeading eyebrow="New expressions" title="The Genesis collection." copy="A consistent featured edit of the first three concept pieces, followed by the complete six-piece collection."/><p className="swipe-hint">Swipe to explore all featured pieces →</p><div className="product-grid featured-products">{products.slice(0, 3).map((product) => <ProductCard key={product.id} product={product} onAdd={onAdd}/>)}</div><Link className="text-link section-link" to="/collections">View the full collection <Icon name="arrow" size={16}/></Link></section>
