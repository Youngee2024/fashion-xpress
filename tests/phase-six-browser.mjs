...
for (const width of [393, 1440]) {
  ...
  await navigate('http://127.0.0.1:4173/collectibles/neo-safari?source=product&step=review')
  let state = await pageState()
  assert.deepEqual(state, { heading: 'Metadata.', path: '/collectibles/neo-safari', overflow: false, broken: 0, focused: true, ethereumReads: 0 })
  ...
  await click('review demo collectible')
  ...
  await click('create demo collectible')
  if (width === 393) {
    await click('cancel demo collectible')
  ...
  assert.deepEqual(state, { heading: 'Demo collectible complete.', path: '/collectibles/neo-safari/complete', overflow: false, broken: 0, focused: true, ethereumReads: 0 })
  ...
  assert.equal((await pageState()).heading, 'Demo collectible complete.')
  await click('view in collectibles lab')
  assert.equal(await evaluate(`document.querySelector('h1')?.innerText`), 'Collectibles Lab.')
